...
export function ChatPanel() {
  const [messages, setMessages] = useState<Message[]>([
    { id: '1', role: 'model', text: 'Hello! I am OrenSymphony. Ask me how to wire up a pipeline, or use the prompt at the top to auto-orchestrate one for you.' },
  ]);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { id: `${Date.now()}-m`, role: 'model', text: responseText || '(empty response)' }]);
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Unknown error';
      setMessages(prev => [...prev, { id: `${Date.now()}-e`, role: 'model', text: `Sorry, I hit an error: ${message}` }]);
    } finally {
      setIsLoading(false);
    }
  };
